...
import {FundingRequest, DChanges, FundingResponse, Funding} from "../../../declarations/dchanges/dchanges.did";
...
export const useCreateFunding = () => {
    return useMutation(
        async (options: {main?: DChanges, req: FundingRequest}) => {
            if(!options.main) {
...
export const useUpdateFunding = () => {
    const queryClient = useQueryClient();
    return useMutation(
        async (options: {main?: DChanges, pubId: string, req: FundingRequest}) => {
            if(!options.main) {
                throw Error('Main actor undefined');
            }
